...
class TestData:
    ...
    def updateDriftNodes(self):
        if self.driftNodes is None:
            ...
            self.driftInterpolator = PchipInterpolator(self.driftNodes,
                                                       self.driftData)
        else:
            while self.pulseId > self.driftNodes[2]:
                self.driftNodes = np.hstack([self.driftNodes[1:],
                                             poisson(self.driftTimescale)
                                             + self.driftNodes[-1]])
                self.driftData = np.hstack([self.driftData[1:],
                                            randn(1)])
                self.driftInterpolator = PchipInterpolator(self.driftNodes,
                                                           self.driftData)
